...
class AdminDashboardController extends Controller
{
    ...
    public function getStats()
    {
        ...
        $hasSchools = Schema::hasTable('schools');
        $hasCourses = Schema::hasTable('courses');
        $hasUserSchoolId = Schema::hasColumn('users', 'school_id');
        $hasCourseSchoolId = $hasCourses && Schema::hasColumn('courses', 'school_id');

        // Total students
        $total_students = User::where('role', 'student')->count();

        // Active subscriptions (real data from subscriptions table)
        $active_subscriptions = $hasSubscriptions ? Subscription::active()->count() : 0;

        // Monthly revenue (real data from payments)
        $monthly_revenue = $hasPayments
        ...
        $pending_payments = $hasPayments
            ? (Payment::whereIn('status', ['pending', 'refunded'])->sum('amount') ?? 0)
            : 0;
        ...
        $current_subs = $hasSubscriptions ? Subscription::active()->count() : 0;
        ...
        $schoolNames = $hasSchools
            ? School::query()->orderBy('id')->pluck('name', 'id')
            : collect();
        ...
        if ($hasUserSchoolId) {
            ...
            $student_distribution = $total_students > 0
                ? [['school_id' => null, 'label' => 'Élèves', 'count' => $total_students]]
                : [];
        }

        // Completed payment totals by school: course.school_id if linked to a course, else payer's school_id
        $revenue_by_school = [];
        if ($hasPayments && $hasCourses && ($hasUserSchoolId || $hasCourseSchoolId)) {
            $aggSchoolExpr = match (true) {
                $hasUserSchoolId && $hasCourseSchoolId => 'COALESCE(courses.school_id, users.school_id)',
                $hasCourseSchoolId => 'courses.school_id',
                default => 'users.school_id',
            };
            ...
            $revenue_by_school = Payment::query()
                ...
                ->selectRaw("{$aggSchoolExpr} as agg_school_id")
                ->selectRaw('SUM(payments.amount) as total')
                ->groupBy(DB::raw($aggSchoolExpr))
                ->get()
                ->map(function ($row) use ($schoolNames) {
        ...
        if ($hasPayments) {
            $recent_payments = Payment::with('user')
                ...
                ->map(function ($payment) {
                    return [
                        'id' => 'PAY'.str_pad((string) $payment->id, 5, '0', STR_PAD_LEFT),
